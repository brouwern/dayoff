...
Some of the biology in this tutorial appears to be out of date.  Specifically, research on unusual frequencies of DNA "words" does not appear to be much current interest in bioinformatics and genomics.  (In contrast, methlation of bases within certain DNA words is being worked on.)  The examples are still good for practicing R skills.
...
```{r}
data("dengueseq_vector_fasta")
```

We can look at this with the str() command

```{r}
str(dengueseq_vector_fasta)
```
...
dengueseq_vector <- fasta_cleaner(dengueseq_vector_fasta, header = header.)
```
...
This means that the DEN-1 Dengue virus genome sequence has 3426 As occuring throughout the genome, 2240 Cs, and so forth.
...
R will automatically try to optimize the appearance of the labels on the greaph so you may not see all of them; no worries.
